...
### ==00 概念==
**转换说明**：
![转换说明](/引用文件/02%20-%202%20-%201.png)
**修饰符**：
...
##### 多变量读取
![[02 - 2 - 1.png]]
scanf函数读取用户输入内容时，要规范用户输入格式，具体如上。==若用户未按照规定格式输入，则会出错==。
读取到的数据，会依次赋值给后面的变量。
```c
int a = 0,b = 0;
scanf("price%d %d",&a,&b);
...
这是因为：在scanf中，空格代表忽略用户输入的**任意数量的空白字符**（包括多个空格、Tab、回车），且%d前也会默认忽略上述。因此，在读取完最后一个整数后，scanf遇到空格，程序会进入忽略用户输入的空白字符状态，直到遇到非空字符。
